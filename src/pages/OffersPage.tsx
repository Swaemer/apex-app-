    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-50 via-white to-gray-100 p-8" dir="rtl">
      <div className="max-w-4xl mx-auto">
        ...
        <div className="mb-8 flex items-center justify-between">
          <div>
            <h1 className="text-4xl font-bold text-gray-900 mb-1">عروض الشهر</h1>
            <p className="text-gray-500">{activeOffer ? activeOffer.month_year : 'لا يوجد عرض نشط'}</p>
          </div>
          {isAdmin && (
        ...
        {isAdmin && showNewOffer && (
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 mb-6">
            <h2 className="text-base font-bold text-gray-900 mb-4">إنشاء عرض جديد</h2>
            <div className="flex gap-3">
              <input type="text" value={newOfferName} onChange={(e) => setNewOfferName(e.target.value)}
                placeholder="مثال: يونيو 2026"
                className="flex-1 px-4 py-2.5 border border-gray-200 rounded-lg text-sm bg-gray-50 focus:outline-none focus:bg-white focus:border-gray-300" />
        ...
        {isAdmin && offers.length > 1 && (
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5 mb-6">
            <h3 className="text-sm font-bold text-gray-700 mb-3">العروض السابقة</h3>
            <div className="flex flex-wrap gap-2">
              {offers.map((offer) => (
                <div key={offer.id} className={`flex items-center gap-2 px-3 py-1.5 rounded-lg border text-sm ${offer.is_active ? 'bg-amber-50 border-amber-200 text-amber-700' : 'bg-gray-50 border-gray-200 text-gray-600'}`}>
                  {offer.is_active && <MdStar className="w-3.5 h-3.5" />}
                  <span>{offer.month_year}</span>
                  {!offer.is_active && (
                    <button onClick={() => handleSetActive(offer.id)} className="text-xs text-blue-600 hover:underline">تفعيل</button>
                  )}
        ...
        {activeOffer ? (
          <div>
            ...
            {isAdmin && (
              <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5 mb-6">
                <h3 className="text-sm font-bold text-gray-700 mb-3">إضافة خدمة جديدة</h3>
                <div className="flex gap-3">
                  ...
                  <input type="number" value={newService.price} onChange={(e) => setNewService((p) => ({ ...p, price: e.target.value }))}
                    ...
                    className="w-36 px-4 py-2.5 border border-gray-200 rounded-lg text-sm bg-gray-50 focus:outline-none focus:bg-white focus:border-gray-300" />
              ...
              <table className="w-full">
                <thead>
                  <tr className="border-b border-gray-200 bg-amber-50">
                    <th className="px-6 py-4 text-right text-sm font-bold text-amber-800">#</th>
                    <th className="px-6 py-4 text-right text-sm font-bold text-amber-800">الخدمة</th>
                    <th className="px-6 py-4 text-right text-sm font-bold text-amber-800">السعر</th>
                    {isAdmin && <th className="px-6 py-4 text-center text-sm font-bold text-amber-800">إجراء</th>}
                  </tr>
                </thead>
                <tbody>
                      ...
                      <td className="px-6 py-4 text-sm">
                        {editingId === s.id ? (
                          <input type="text" value={editValues.name} onChange={(e) => setEditValues((p) => ({ ...p, name: e.target.value }))}
                            className="w-full px-3 py-1.5 border border-blue-300 rounded-lg text-sm focus:outline-none bg-white" />
                        ) : (
                      ...
                      <td className="px-6 py-4 text-sm">
                        {editingId === s.id ? (
                          <input type="number" value={editValues.price} onChange={(e) => setEditValues((p) => ({ ...p, price: e.target.value }))}
                            className="w-28 px-3 py-1.5 border border-blue-300 rounded-lg text-sm focus:outline-none bg-white" />
                        ) : (
                          <span className="font-bold text-amber-600">{s.price.toLocaleString('ar-SA')} ريال</span>
                        )}
                      </td>
                      {isAdmin && (
                        <td className="px-6 py-4 text-center">
                          {editingId === s.id ? (
                            <div className="flex gap-1 justify-center">
                              <button onClick={() => handleUpdateService(s.id)} className="p-1.5 text-green-600 hover:bg-green-50 rounded-lg"><MdCheck className="w-4 h-4" /></button>
                              <button onClick={() => setEditingId(null)} className="p-1.5 text-gray-400 hover:bg-gray-100 rounded-lg"><MdClose className="w-4 h-4" /></button>
                            </div>
                          ) : (
                            <div className="flex gap-1 justify-center">
                              <button onClick={() => { setEditingId(s.id); setEditValues({ name: s.service_name, price: String(s.price) }); }}
                                className="p-1.5 text-blue-500 hover:bg-blue-50 rounded-lg"><MdEdit className="w-4 h-4" /></button>
                              <button onClick={() => handleDeleteService(s.id)} className="p-1.5 text-red-500 hover:bg-red-50 rounded-lg"><MdDelete className="w-4 h-4" /></button>
                            </div>
                          )}
                        </td>
                      )}
          ...
          <div className="bg-white rounded-2xl border border-dashed border-gray-300 p-16 text-center">
            <p className="text-gray-400 text-lg">لا يوجد عرض نشط</p>
            {isAdmin && <p className="text-gray-400 text-sm mt-2">اضغط "عرض جديد" لإنشاء عروض الشهر</p>}
          </div>
        )}
